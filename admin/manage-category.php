...
if(isset($_POST['form1']))
{
	try {
		...
		$statement = $db->prepare("SELECT * FROM tbl_categories WHERE cat_name=?");
		...
		$statement = $db->prepare("INSERT INTO tbl_categories (cat_name) VALUES (?)");
		$statement->execute(array($_POST['cat_name']));
...
if(isset($_POST['form2']))
{
	try {
		...
		$statement = $db->prepare("UPDATE tbl_categories SET cat_name=? WHERE cat_id=?");
...
if(isset($_REQUEST['id'])) 
{
	...
	$statement = $db->prepare("DELETE FROM tbl_categories WHERE cat_id=?");
